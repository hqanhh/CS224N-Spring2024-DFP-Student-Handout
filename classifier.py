...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch}: train loss :: {train_loss :.3f}, train acc :: {train_acc :.3f}, dev acc :: {dev_acc :.3f}, epoch time :: {epoch_end_time - epoch_start_time :.3f} seconds")
...
def test(args):
    with torch.no_grad():
        ...
        print(f'DONE DEV in {eval_end_time - eval_start_time :.3f} seconds')

        test_start_time = time.time()
        test_pred, test_sents, test_sent_ids = model_test_eval(test_dataloader, model, device)
        test_end_time = time.time()
        print(f'DONE Test in {test_end_time - test_start_time :.3f} seconds')
